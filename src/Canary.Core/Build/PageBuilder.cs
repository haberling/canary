...
// "Incremental builds" section) -- when the source's checksum matches what's
// embedded in the existing output file, the previous render's #app fragment
// is extracted and reused instead of re-running the markdown renderer.
public sealed partial class PageBuilder
{
    ...
    public PageBuildResult BuildPage(string sourceMarkdownPath, string outputHtmlPath, string shellTemplate, string siteName, string widgetScriptsHtml = "")
    {
        ...
        {
            content = _markdownRenderer.Render(source);
        ...
        var checksumComment = $"<!-- source-checksum: sha256:{checksum} -->";
    ...
    [GeneratedRegex(@"<!--\s*source-checksum:\s*sha256:([0-9a-f]{64})\s*-->")]

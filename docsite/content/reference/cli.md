...
Run from a checkout as `dotnet run --project src/Canary -- <command> [args]`. Every command except `init` and `docs` takes `--config <path>` (default `canary.jsonc` in the current directory) to point at a specific site.
